"""OpenAI client for SQL generation and insights (mirrors claude_client.py)."""
...
def _strip_model(model: str) -> str:
    """Remove 'openai:' prefix to get the actual model name."""
...
def generate_sql(question: str, model: str, customer_ids: list[int] | None = None) -> tuple[str, dict]:
    ...
    response = client.chat.completions.create(
        model=_strip_model(model),
        max_completion_tokens=1024,
        temperature=0,
        messages=[
            {
                "role": "system",
                "content": f"""SQL expert for DuckDB. Generate one SELECT query answering the question.
...
Always use table aliases and qualify every column with its alias to avoid ambiguous references.{customer_constraint}""",
            ...
            {"role": "user", "content": question + hints},
        ],
    )
    return _clean_sql(response.choices[0].message.content), _usage(response)
...
def fix_sql(question: str, sql: str, error: str, model: str) -> tuple[str, dict]:
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": f"""SQL expert for DuckDB. Fix the SQL query that failed.
...
def generate_insight(question: str, sql: str, columns: list[str], data: list[list], model: str) -> tuple[dict, dict]:
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": """Analyze SQL results for restaurant dashboard. Return JSON only:
...
def generate_dashboard_queries(model: str) -> list[dict]:
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": f"""Restaurant analytics expert. Given schema, generate 4 dashboard cards.
